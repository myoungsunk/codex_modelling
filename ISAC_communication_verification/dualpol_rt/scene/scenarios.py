...
class _LayoutBuilder:
    ...
    def _resolve_material(self, material: str | Material) -> Material:
        return material_named(material) if isinstance(material, str) else material
    ...
    def add_shell(self) -> None:
        lx, ly, lz = self.spec.room_size
        mats = material_bundle(self.spec.material_preset)
...
def _open_office_builder(spec: ScenarioSpec) -> _ScenarioLayout:
    builder = _LayoutBuilder(spec)
    builder.add_shell()
...
def _mixed_office_builder(spec: ScenarioSpec) -> _ScenarioLayout:
    builder = _LayoutBuilder(spec)
    builder.add_shell()
...
def _lecture_room_builder(spec: ScenarioSpec) -> _ScenarioLayout:
    builder = _LayoutBuilder(spec)
    builder.add_shell()
...
def _straight_corridor_builder(spec: ScenarioSpec) -> _ScenarioLayout:
    builder = _LayoutBuilder(spec)
    builder.add_shell()
...
def _factory_lite_builder(spec: ScenarioSpec) -> _ScenarioLayout:
    builder = _LayoutBuilder(spec)
    builder.add_shell()
...
def _l_corridor_proxy_builder(spec: ScenarioSpec) -> _ScenarioLayout:
    builder = _LayoutBuilder(spec)
    builder.add_shell()
...
def _lobby_blocker_proxy_builder(spec: ScenarioSpec) -> _ScenarioLayout:
    builder = _LayoutBuilder(spec)
    builder.add_shell()
...
def build_scenario(spec: str | ScenarioSpec) -> Scene:
    return build_scenario_layout(spec).scene


def build_scenario_layout(spec: str | ScenarioSpec) -> _ScenarioLayout:
    ...
    return builder(use_spec)


def scenario_grid(spec: str | ScenarioSpec, grid_step: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:

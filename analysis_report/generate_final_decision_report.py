...
def _build_final_markdown(
    *,
    ...
    lines.append(
        ...
        "(A3_supp/A4_bridge WARN are intentional scope-control flags)."
    )
    lines.append("")
    ...
    lines.append("- A3_supp: mechanism validation only (supplementary when A6 is present)")
    lines.append("- A4_bridge: bridge/support evidence only (secondary, not primary material sign-off)")
    lines.append("- A5_pair: contamination-response analysis only (paired base/on)")
    ...
    lines.append("| A6_on | bridge observability | WARN | LOS-on bridge check | G2 sign-off |")
    lines.append("| A3_supp | supplementary mechanism | WARN(scope-control) | mechanism-only context | system early baseline / G2 sign-off |")
    if bool(a6_parity.get("available", False)):
    ...
    lines.append("| A4_bridge | material effect secondary support | WARN(scope-control) | bridge/support context | primary material claim |")
    lines.append("| A4_on | bridge observability | WARN | LOS-on bridge check | L2-M sign-off |")
    lines.append("| A5_pair | contamination-response pair | PASS | paired contamination/stress-response evidence | faithful rough/human solver claim |")
    ...
    lines.append("1. Freeze A3_supp as mechanism-only scenario (WARN(scope-control))")
    lines.append("2. Freeze A4_bridge as bridge/support-only scenario (WARN(scope-control))")
    lines.append("3. Freeze A5_pair as contamination-response scenario")
    lines.append("4. Document `LOS0_q3` as structural hole")
    lines.append("5. Keep `qNA_total = 0` in frozen diagnostic output")
    lines.append("6. Archive per-scenario summary (`B_per_scenario_summary.csv`)")
    lines.append("7. Document B1/B2/B3 interpretation as coverage-aware leverage map (not absolute universal map)")
    lines.append("")
    ...
    lines.append("- Remaining WARN items are interpretation/documentation scope (scope-control, not weakness)")
    lines.append("")
    ...
    for sid in ["C0", "A2", "A2_on", "A3", "A3_on", "A4", "A4_on", "A5", "A6", "A6_on", "B1", "B2", "B3"]:
...
def main() -> None:
    ...
    ap.add_argument("--scenarios", default="C0,A2,A2_on,A3,A3_on,A4,A4_on,A5,A6,A6_on,B1,B2,B3")
    args = ap.parse_args()

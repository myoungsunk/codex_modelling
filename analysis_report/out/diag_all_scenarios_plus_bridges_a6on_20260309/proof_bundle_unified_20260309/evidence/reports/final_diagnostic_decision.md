...
- Experiment tag: `diag_all_scenarios_plus_bridges_a6on_20260309`
- Reference artifacts:
...
- A5 target mode=contamination_response, W3 (room Te sweep) status=WARN (best S_xpd_early=0.737)
...
### Reporting rule
- A3: mechanism validation only (supplementary when A6 is present)
...
### Evidence
- delta_ref=0.449 dB (floor_delta=0.449 dB, repeat_delta=nan dB)
- A3-A2 early delta_median=0.000 dB (ratio=0.000, C1=FAIL)
- A4_iso primary span=0.801 dB (WARN, n=12)
- A4_bridge secondary span=0.099 dB (FAIL, n=12)
- A4_bridge dispersion_on count=6 (claim_ready=True)
- Stress primary effect ΔL_pol=0.573 dB (FAIL, C2S=FAIL)
...
### Evidence
- D1-global=PASS (EL_iqr=7.709 dB)
- D1-local(A2)=PASS, D1-local(A5)=FAIL role=stress_response_proxy
- D2(stage1)=WARN, D2(stage2)=PASS (overall=FAIL)
...
| A3_corner | supplementary mechanism | WARN | mechanism-only context | system early baseline / G2 sign-off |
...
| A4_bridge | material effect secondary | WARN | L2-M bridge/late sensitivity | primary material claim |
| A4_on | bridge observability | WARN | LOS-on bridge check | L2-M sign-off |
| A5_pair | proxy stress response | PASS | L2-S primary (synthetic) + geometric sensitivity | faithful rough/human solver claim |
...
| B3 | corner high-EL NLOS | FAIL | coverage-aware high-EL stress leverage | absolute universal map claim |
...
6. Document B1/B2/B3 interpretation as coverage-aware leverage map (not absolute universal map)
...
- Floor reference used: median=25.063 dB, delta=0.449 dB

...
def generate_rt_vs_synth_plots(
    ...
    ax.set_title("F5 RT vs Synthetic subband XPD (mu/sigma)")
    ...
    ph_rt = offdiag_phases(rt_paths, matrix_source=matrix_source)
    ph_sy = offdiag_phases(synth_paths, matrix_source=matrix_source)
    ...
    axs[0].set_title(f"RT offdiag phase\nKuiper V={ku_rt['V']:.3f}, p={ku_rt['p_boot']:.3f}")
    axs[0].set_xlabel("phase [rad]")
    axs[0].set_ylabel("density")
    axs[0].grid(True, alpha=0.3)
    axs[1].hist(ph_sy, bins=bins, density=True, alpha=0.7, label="Synthetic")
    axs[1].set_title(f"Synth offdiag phase\nKuiper V={ku_sy['V']:.3f}, p={ku_sy['p_boot']:.3f}")
    ...
    return {
        ...
        "f3_xpd_mu_delta_abs_db": float(abs(np.mean(x_rt) - np.mean(x_sy))) if len(x_rt) and len(x_sy) else np.nan,

...
from dualpol_rt.feature_extractor import extract_all_features
...
def build_phase2_patterns(
    ...
    return {
        ...
        "tx_cp": make_realistic_patch_cp_antenna(
            tx_cp_files,
            handedness="right",
            pos=tx_pose_use.origin,
            boresight=tx_pose_use.z_axis,
            name="dual_cp_tx",
            normalization_mode=normalization_mode,
        ),
        "rx_cp": make_realistic_patch_cp_antenna(
            rx_cp_files,
            handedness="right",
            pos=rx_pose_use.origin,
            boresight=rx_pose_use.z_axis,
            name="dual_cp_rx",
            normalization_mode=normalization_mode,
        ),
    }
...
def evaluate_realistic_debug_point(
    ...
    scene = build_shoebox(
        ...
        material_preset=cfg.material_preset,
        material_xpol_coupling_db=cfg.material_xpol_coupling_db,
    )
    ...
    return {
        ...
        "lp_raw_features": extract_all_features({"H_linear": H_lp_raw, "freqs_hz": cfg.freqs_hz}),
        "cp_raw_features": extract_all_features({"H_circular": H_cp_raw, "freqs_hz": cfg.freqs_hz}),
        "lp_normalized_features": extract_all_features({"H_linear": H_lp_norm, "freqs_hz": cfg.freqs_hz}),
        "cp_normalized_features": extract_all_features({"H_circular": H_cp_norm, "freqs_hz": cfg.freqs_hz}),

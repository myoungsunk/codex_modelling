...
class IndoorCampaignTests(unittest.TestCase):
    ...
    def test_campaign_reports_delta_rate_p05_as_percentile_difference(self) -> None:
        ...
        self.assertIn("lp_xpr_raw_db_mean", row)
        self.assertIn("cp_condition_number_raw_mean", row)
        self.assertIn("lp_pdg_norm_mean", row)
        self.assertIn("cp_mimo_efficiency_norm_mean", row)
        ...
        self.assertNotEqual(row["delta_xpr_norm_db_mean"], 0.0)
        self.assertNotEqual(row["delta_pdg_raw_mean"], 0.0)
    ...
    def test_campaign_exports_capacity_columns_and_maps(self) -> None:
        ...
        with (output_dir / "scene_metrics.csv").open("r", encoding="utf-8", newline="") as handle:
            ...
            self.assertIn("lp_xpr_raw_db", reader.fieldnames)
            self.assertIn("cp_condition_number_norm", reader.fieldnames)
            self.assertIn("delta_pdg_raw_snr_10", reader.fieldnames)
            self.assertIn("lp_mimo_efficiency_norm_snr_10", reader.fieldnames)
            first_row = next(reader)
            self.assertIn("lp_capacity_raw_snr_10", first_row)
            self.assertIn("lp_capacity_norm_snr_10", first_row)
            self.assertIn("delta_xpr_raw_db", first_row)
            self.assertIn("delta_condition_number_norm", first_row)

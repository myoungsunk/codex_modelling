...
from dualpol_rt.patterns import AntennaPose, FFDPattern, make_ideal_cp_antenna
...
class FFDPatternTests(unittest.TestCase):
    ...
    def test_make_ideal_cp_antenna_exposes_expected_metadata(self) -> None:
        antenna = make_ideal_cp_antenna(handedness="right", pos=np.array([0.0, 0.0, 0.0], dtype=float), boresight=np.array([1.0, 0.0, 0.0], dtype=float))
        self.assertEqual(antenna.antenna_mode, "ideal_cp")
        self.assertEqual(antenna.port_labels, ("RHCP", "LHCP"))
        self.assertEqual(antenna.basis, "circular")
        self.assertFalse(antenna.enable_coupling)
        self.assertEqual(antenna.cross_pol_leakage_db, 60.0)
        self.assertAlmostEqual(float(np.linalg.norm(antenna.pose.z_axis - np.array([1.0, 0.0, 0.0], dtype=float))), 0.0, places=12)

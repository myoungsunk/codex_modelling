...
checkpoint_path = fullfile(out_dir, 'stage2_900_ffd_det_checkpoint.mat');
mat_path = fullfile(out_dir, 'stage2_900_ffd_det.mat');
csv_path = fullfile(out_dir, 'stage2_900_ffd_det.csv');
summary_path = fullfile(out_dir, 'stage2_900_ffd_det_summary.md');
...
if exist(checkpoint_path, 'file') == 2
    ...
    if isfield(C, 'cases_saved') && height(C.cases_saved) == n && isequal(C.cases_saved.case_id, cases.case_id)
        ...
        fprintf('Resuming deterministic Stage 2 full sweep from case %d of %d\n', start_idx, n);
    else
        fprintf('Deterministic checkpoint exists but case layout differs. Starting fresh.\n');
    end
end
...
fprintf(fid, '# Stage 2 Full Sweep (Deterministic)\n\n');
fprintf(fid, '- cases: %d\n', height(cases));
...
fprintf('Stage 2 deterministic full: %.1f min (%.1f ms/case)\n', elapsed / 60.0, elapsed / height(cases) * 1000.0);

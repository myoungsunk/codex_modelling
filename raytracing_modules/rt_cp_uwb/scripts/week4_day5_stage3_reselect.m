...
mat_path = fullfile(stage2_dir, 'stage2_900_ffd_relabel.mat');
if exist(mat_path, 'file') ~= 2
    run(fullfile(repo_root, 'scripts', 'week4_day4_stage2_relabel_rerun.m'));
end
...
cells_csv = fullfile(stage3_dir, 'stage3_reselection_cells.csv');
cases_csv = fullfile(stage3_dir, 'hfss_case_list.csv');
md_path = fullfile(stage3_dir, 'stage3_reselection.md');
writetable(cells_tbl, cells_csv);
writetable(cases_tbl, cases_csv);

fid = fopen(md_path, 'w');
cleanup_obj = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, '# Stage 3 HFSS 30-Case Reselection\n\n');
fprintf(fid, '## Principle\n\n');
fprintf(fid, '- Total cases: 30\n');
fprintf(fid, '- GEO group: 15 cases (3 regimes x 5 cases)\n');
fprintf(fid, '- BOUNCE group: 15 cases (3 regimes x 5 cases)\n');
fprintf(fid, '- Selection basis: Stage 2 relabeled dataset with dual-label decomposition.\n');
fprintf(fid, '- Reporting label remains `mixed@0.33`, but Stage 3 selection is split into physical sub-regimes.\n\n');

fprintf(fid, '## Selected Regimes\n\n');
fprintf(fid, '| group | regime | n | n_neg | n_pos | auc_cir | auc_joint | delta_auc |\n');
fprintf(fid, '|---|---|---:|---:|---:|---:|---:|---:|\n');
for idx = 1:height(cells_tbl)
    fprintf(fid, '| %s | %s | %d | %d | %d | %s | %s | %s |\n', ...
        cells_tbl.group{idx}, cells_tbl.regime_desc{idx}, cells_tbl.n(idx), cells_tbl.n_neg(idx), cells_tbl.n_pos(idx), ...
        fmt(cells_tbl.auc_cir(idx)), fmt(cells_tbl.auc_joint(idx)), fmt(cells_tbl.delta_auc(idx)));
end

fprintf(fid, '\n## Outputs\n\n');
fprintf(fid, '- Cell summary: `%s`\n', cells_csv);
fprintf(fid, '- HFSS case list: `%s`\n', cases_csv);

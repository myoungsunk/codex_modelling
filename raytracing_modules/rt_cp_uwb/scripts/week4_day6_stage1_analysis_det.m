...
matPath = fullfile(outDir, 'stage1_3000_ffd_det.mat');
if exist(matPath, 'file') ~= 2
    run(fullfile(projectRoot, 'scripts', 'week4_day6_stage1_full_det.m'));
end
...
writetable(global_auc, fullfile(outDir, 'global_auc_summary_det.csv'));
...
plotPath = fullfile(outDir, 'conditional_auc_heatmaps_det.png');
saveas(fig, plotPath);
close(fig);

cell_table_all = vertcat(cell_tables{:});
writetable(cell_table_all, fullfile(outDir, 'conditional_auc_cells_det.csv'), 'WriteVariableNames', true);
save(fullfile(outDir, 'conditional_auc_heatmaps_det.mat'), 'pair_results', 'global_auc', 'cp_features', 'cir_features', 'joint_features', 'cell_table_all');

disagreement = analysis.computeDisagreementAnalysis(results, cir_features, cp_features);
save(fullfile(outDir, 'disagreement_analysis_det.mat'), 'disagreement');

mdPath = fullfile(outDir, 'conditional_auc_summary_det.md');
fid = fopen(mdPath, 'w');
assert(fid ~= -1, 'Failed to open %s', mdPath);
cleanupObj = onCleanup(@() fclose(fid)); %#ok<NASGU>
fprintf(fid, '# Stage 1 Conditional AUC Summary (Deterministic)\n\n');
...
fprintf('Saved:\n');
fprintf('  %s\n', plotPath);
fprintf('  %s\n', fullfile(outDir, 'conditional_auc_cells_det.csv'));
fprintf('  %s\n', mdPath);

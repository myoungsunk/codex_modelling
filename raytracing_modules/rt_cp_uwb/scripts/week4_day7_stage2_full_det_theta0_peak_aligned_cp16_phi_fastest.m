...
cfg.stage2_ffd_peak_align_local_posz = true;
cfg.stage2_ffd_peak_align_note = 'Map local FFD theta=0,phi=90 (+z) to Tx -z and Rx +z for Stage 2 patch_ffd cases.';
cfg.stage2_ffd_peak_local_theta_deg = 0.0;
cfg.stage2_ffd_peak_local_phi_deg = 90.0;
cfg.stage2_ffd_tx_peak_world_target = [0; 0; -1];
cfg.stage2_ffd_rx_peak_world_target = [0; 0; 1];
cfg.stage2_ffd_sample_order_policy = 'loadFfdPattern(sample_order=auto), expected resolved_sample_order=phi_fastest';
cfg.stage2_orientation_variant = 'theta0_phi90_local_posz_to_tx_minus_z_rx_plus_z';
cfg.enable_cp16_features = true;

out_dir = fullfile(repo_root, 'results', 'stage2_theta0_peak_aligned_cp16_phi_fastest');
if exist(out_dir, 'dir') ~= 7
    mkdir(out_dir);
end

cases = sweep.designStage2Cases(300, 42);
stem = 'stage2_900_theta0_peak_aligned_cp16_phi_fastest_det';
max_cases_env = string(getenv('STAGE2_THETA0_MAX_CASES'));
if strlength(max_cases_env) > 0
    ...
    if isfinite(max_cases) && max_cases > 0 && max_cases < height(cases)
        ...
        stem = sprintf('stage2_smoke_%d_theta0_peak_aligned_cp16_phi_fastest_det', height(cases));
    end
end

runSweep(cases, cfg, out_dir, stem, 25);

function runSweep(cases, cfg, out_dir, stem, checkpoint_every)
    ...
    if exist(checkpoint_path, 'file') == 2
        ...
        if isfield(C, 'cases_saved') && height(C.cases_saved) == n && isequal(C.cases_saved.case_id, cases.case_id) && ...
            ...
            cfg = C.cfg_saved;
            fprintf('Resuming Stage 2 theta0 sweep from case %d of %d\n', start_idx, n);
        elseif isfield(C, 'cases_saved') && height(C.cases_saved) == n && isequal(C.cases_saved.case_id, cases.case_id)
            fprintf('Ignoring Stage 2 checkpoint because saved cfg signature differs from requested cfg.\n');
        end
    end
...
function tf = checkpointCfgMatches(requested_cfg, saved_cfg)
    fields = { ...
        'seed_base', ...
        'seed_stage_id', ...
        'stage2_ffd_peak_align_local_posz', ...
        'stage2_ffd_peak_local_theta_deg', ...
        'stage2_ffd_peak_local_phi_deg', ...
        'stage2_orientation_variant', ...
...
function [rhcp_order, lhcp_order] = probeFfdSampleOrder(repo_root)
    ...
    if exist(rhcp_path, 'file') == 2
        ffd = antennas.loadFfdPattern(rhcp_path, 'notes', 'stage2_theta0_summary_probe');
        rhcp_order = string(ffd.metadata.sample_order);
    end
    if exist(lhcp_path, 'file') == 2
        ffd = antennas.loadFfdPattern(lhcp_path, 'notes', 'stage2_theta0_summary_probe');
...
function writeSummary(path, out_dir, results, elapsed, cfg, rhcp_order, lhcp_order, checkpoint_every, failed_path)
    ...
    fprintf(fid, '# Stage 2 Full Sweep (Theta0 Peak-Aligned FFD + CP16)\n\n');
    fprintf(fid, '- cases: %d\n', height(results));
    ...
    fprintf(fid, '- tx_boresight: [0;0;-1]\n');
    fprintf(fid, '- rx_boresight: [0;0;1]\n\n');
    room_values = string(resolveColumn(results, 'room_type'));
    has_los = logical(resolveColumn(results, 'has_los_path'));
    ratio = double(resolveColumn(results, 'bounce_to_los_ratio_mid'));
    rooms = unique(room_values);
    fprintf(fid, '| room | n_total | n_valid | n_failed | mixed_0p33_pos |\n');
    fprintf(fid, '|---|---:|---:|---:|---:|\n');
    for i = 1:numel(rooms)
        m = room_values == rooms(i);
        mixed = (~has_los) | (has_los & ratio >= 0.33);
        fprintf(fid, '| %s | %d | %d | %d | %d |\n', rooms(i), sum(m), sum(m & valid), sum(m & ~valid), sum(m & valid & mixed));
    end
end
